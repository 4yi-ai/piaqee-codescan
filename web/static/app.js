...
(function () {
  ...
  function syncTopCardsHeight() {
    var mainCard = document.querySelector(".page-main > .card");
    var sideCard = document.querySelector(".page-side");
    if (!mainCard || !sideCard) return;
    mainCard.style.minHeight = "";
    if (window.innerWidth <= 980) return;
    mainCard.style.minHeight = sideCard.offsetHeight + "px";
  }
  ...
  function setHostsMessage(msg, ok) {
    var box = document.getElementById("hosts-msg");
    if (!box) return;
    box.textContent = msg;
    box.classList.remove("hidden");
  ...
  function updateAllowedHosts(hosts) {
    var text = hosts.join(", ");
    document.querySelectorAll("[data-allowed-hosts]").forEach(function (el) {
      el.textContent = text;
    });
  }

  function updateSavedHosts(hosts) {
    var list = document.getElementById("saved-hosts-list");
    var count = document.getElementById("saved-count");
    var badge = document.getElementById("saved-badge");
    if (count) count.textContent = String(hosts.length);
    if (badge) badge.textContent = hosts.length > 0 ? (hosts.length + " saved") : "Saved in app";
    if (!list) return;
    list.innerHTML = "";
    if (!hosts.length) {
      var empty = document.createElement("span");
      empty.id = "saved-hosts-empty";
      empty.className = "muted small";
      empty.textContent = "No extra hosts saved yet.";
  ...
  var hostsForm = document.getElementById("hosts-form");
  if (hostsForm) {
    hostsForm.addEventListener("submit", function (e) {
      e.preventDefault();
      var btn = submitBtn(hostsForm);
      var original = btn ? btn.textContent : "";
      var fd = new FormData(hostsForm);
      if (btn) {
        btn.disabled = true;
        btn.textContent = "Saving…";
      }
      fetch("/api/settings/allowed-hosts", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ hosts: (fd.get("hosts") || "").trim() })
      })
        .then(function (res) {
          return res.json().catch(function () { return {}; }).then(function (data) {
        ...
        .then(function (data) {
          var savedHosts = data.extra_allowed_hosts || [];
          updateAllowedHosts(data.allowed_hosts || []);
          updateSavedHosts(savedHosts);
          var input = hostsForm.querySelector('[name="hosts"]');
          if (input) input.value = savedHosts.join("\n");
          setHostsMessage("Saved " + savedHosts.length + " host" + (savedHosts.length === 1 ? "" : "s") + ".", true);
          syncTopCardsHeight();
        })
        .catch(function (err) {
          setHostsMessage(err.error || "Could not save allowed hosts.", false);
          syncTopCardsHeight();
        })
        .finally(function () {
          if (btn) {
  ...
  syncTopCardsHeight();
  window.addEventListener("resize", syncTopCardsHeight);
    ...
    var timer = setInterval(function () {
      fetch("/api/scans/" + id)
        ...
        .then(function (job) {
          ...
          if (prog) prog.textContent = terminal ? "" : (job.progress || "");
          if (terminal) {
            clearInterval(timer);
