...
func TestParseAllowedHostsCSV(t *testing.T) {
	got, err := ParseAllowedHostsCSV(" GitLab.Bieases.com, git.internal.com ,gitlab.bieases.com ")
	if err != nil {
		t.Fatalf("ParseAllowedHostsCSV: %v", err)
	}
	want := []string{"gitlab.bieases.com", "git.internal.com"}
	if len(got) != len(want) {
		t.Fatalf("host count mismatch: got %v want %v", got, want)
	}
	for i := range want {
		if got[i] != want[i] {
...
func TestParseAllowedHostsCSVAcceptsRepoURLs(t *testing.T) {
	got, err := ParseAllowedHostsCSV("https://gitlab.bieases.com/dop/bieases-wallet.git, https://git.internal.com/group/repo")
	if err != nil {
		t.Fatalf("ParseAllowedHostsCSV: %v", err)
	}
	want := []string{"gitlab.bieases.com", "git.internal.com"}
...
func TestParseAllowedHostsCSVAcceptsNewlines(t *testing.T) {
	got, err := ParseAllowedHostsCSV("gitlab.bieases.com\nhttps://git.internal.com/group/repo\r\nanother.git.local")
	if err != nil {
		t.Fatalf("ParseAllowedHostsCSV: %v", err)
	}
	want := []string{"gitlab.bieases.com", "git.internal.com", "another.git.local"}
	if len(got) != len(want) {
		t.Fatalf("host count mismatch: got %v want %v", got, want)
	}
	for i := range want {
...
func TestParseAllowedHostsCSVRejectsInvalid(t *testing.T) {
	for _, raw := range []string{
		"http://gitlab.bieases.com/group/repo",
		"gitlab.bieases.com:443",
		"https://gitlab.bieases.com:443/group/repo",
		"https://user:pass@gitlab.bieases.com/group/repo",
		"-bad.example.com",
	} {
		if _, err := ParseAllowedHostsCSV(raw); err == nil {
