...
func (s *Server) createGitScan(w http.ResponseWriter, r *http.Request) {
	...
	guards, err := s.currentGuards(r.Context())
	if err != nil {
		writeErr(w, http.StatusInternalServerError, "could not load allowed hosts")
		return
	}
	cleanURL, err := source.SanitizeGitURL(req.SourceRef, guards)
	if err != nil {
	...
	s.enqueueJob(w, r, store.SourceGit, cleanURL, strings.TrimSpace(req.Branch), scan.Secret{Token: token, Branch: strings.TrimSpace(req.Branch)})
}
...
func (s *Server) enqueueJob(w http.ResponseWriter, r *http.Request, sourceType, ref, branch string, sec scan.Secret) {
	id, err := newID()
	if err != nil {
		writeErr(w, http.StatusInternalServerError, "id generation failed")
		return
	}
	s.enqueueJobWithID(w, r, id, sourceType, ref, branch, sec)
}

// enqueueJobWithID creates the job row and enqueues it with its in-memory secret.
func (s *Server) enqueueJobWithID(w http.ResponseWriter, r *http.Request, id, sourceType, ref, branch string, sec scan.Secret) {
	job, err := s.store.CreateJob(r.Context(), id, sourceType, ref, branch)
	if err != nil {
