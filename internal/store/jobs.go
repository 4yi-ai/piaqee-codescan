...
type Finding struct {
	...
	CWE          string `json:"cwe,omitempty"`          // CWE id(s), comma-separated (e.g. "CWE-89")
...
func (s *Store) CreateJob(ctx context.Context, id, sourceType, sourceRef, branch string) (*Job, error) {
	j := &Job{
		ID:         id,
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO jobs (id, source_type, source_ref, branch, status, created_at)
		 VALUES (?, ?, ?, ?, ?, ?)`,
		j.ID, j.SourceType, j.SourceRef, j.Branch, j.Status, j.CreatedAt)
	if err != nil {
...
func (s *Store) GetJob(ctx context.Context, id string) (*Job, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, source_type, source_ref, branch, status, progress, error, summary,
...
// CountJobs returns the total number of jobs.
func (s *Store) CountJobs(ctx context.Context) (int, error) {
	row := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM jobs`)
	var n int
...
// ListJobsPage returns jobs newest-first using limit/offset pagination.
...
func scanJob(row rowScanner) (*Job, error) {
	...
	if err := row.Scan(&j.ID, &j.SourceType, &j.SourceRef, &branch, &j.Status,
...
func (s *Store) InsertFindings(ctx context.Context, fs []Finding) error {
	...
	stmt, err := tx.PrepareContext(ctx,
		`INSERT INTO findings (job_id, tool, category, severity, rule_id, title,
		    message, file_path, line, pkg_name, pkg_ver, fixed_ver, cve, cwe, relationship, usage, raw)
		 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
	if err != nil {
	...
	for i := range fs {
		...
		if _, err := stmt.ExecContext(ctx, f.JobID, f.Tool, f.Category, f.Severity,
			...
			f.FixedVer, f.CVE, f.CWE, f.Relationship, f.Usage, f.Raw); err != nil {
...
func (s *Store) ListFindings(ctx context.Context, jobID string, f FindingFilter) ([]Finding, error) {
	...
	query := `SELECT id, job_id, tool, category, severity, rule_id, title, message,
	                 file_path, line, pkg_name, pkg_ver, fixed_ver, cve, cwe, relationship, usage, raw
	...
	for rows.Next() {
		...
		f.CVE, f.CWE = cve.String, cwe.String
		f.Relationship, f.Usage, f.Raw = relationship.String, usage.String, raw.String
		out = append(out, f)
	}
	return out, rows.Err()
}
